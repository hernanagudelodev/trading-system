...
async def _fetch_option_data(session, ticker, price, strategy):
    ...
    try:
        ...
        # El lado (put/call) sale de la ESTRUCTURA, no del signo del débito: Bear Put
        # es débito pero necesita puts; Bear Call es crédito pero necesita calls.
        es_put = strategy in ("Bull Put Spread", "Bear Put Spread", "Long Put")
        if es_put:
