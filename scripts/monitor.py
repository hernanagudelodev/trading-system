...
Alert levels (v2):
    NORMAL  → within expected ranges       → console
    WATCH   → approaching targets          → console + Telegram
    ACTION  → take profit reached          → console + Telegram
    URGENT  → stop loss / max profit       → console + Telegram
El P&L de cada posicion se escribe a la DB (paper_positions/positions); el
dashboard (servicio aparte) lo lee de ahi. El monitor NO genera HTML —
las notificaciones van por Telegram (notify.send_push), no ntfy.
...
try:
    ...
    schedule = None   # v2: solo scheduled_run lo usa
...
try:
    ...
    def send_push(*args, **kwargs):   # v2: sin ntfy — alertas desactivadas
        return None
...
def get_live_option_price(ticker, strike, expiration, option_type="call"):
    """
    Mid de UNA opción (long). v2 usa pricing.get_single_value (DXLink, Tastytrade)
    en vez de yfinance (eliminado del flujo v2). Devuelve el mid o None.
    """
    import pricing
    return pricing.get_single_value(ticker, strike, expiration, option_type)
