...
def get_historical_criteria(ticker, as_of_date, full_data):
    ...
    This is the key function that makes point-in-time backtesting possible:
    instead of downloading fresh data, we slice the pre-downloaded dataset.
    ...
    # This is a known limitation of the free backtest approach.
    # IV is approximated as HV * 1.1 (slight premium over historical)
    ...
    # ── Fundamentals — use current values (limitation noted) ─────────────────
    # Historical PE/margins not available without paid API.
    # We use current fundamentals as approximation.
    # This slightly biases the backtest but is acceptable for initial calibration.
    fundamentals = _get_fundamentals_cached(ticker)

    # ── Beta — use current value ──────────────────────────────────────────────
    beta = _get_beta_cached(ticker)

    # ── PCR and OI — not available historically ───────────────────────────────
    # These criteria will score 0 (neutral) in the backtest
    # This is a known limitation

    # ── Earnings — check if earnings occurred within 20 days of as_of_date ───
...
def _get_earnings_near_date(ticker, as_of_date):
    """
    Check if there's an earnings event within the next 35 days
    from as_of_date using yfinance calendar data.
    Cached per ticker.
    """
    ...
    # Find nearest earnings date relative to as_of_date
    future_dates = [
        d for d in cached["dates"]
        if d > as_of_date
    ]
...
def run_backtest(tickers, lookback_days=DEFAULT_LOOKBACK_DAYS, resume=False):
    """
    ...
    Pipeline per ticker:
        ...
        3. For each trading day:
            ...
            c. Score criteria
            d. Save to DB with is_backtest=True
    ...
    for ticker in tickers:
        ...
        # ── Download full historical data for this ticker ─────────────────────
        ...
        for sim_date in trading_days:
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    analysis_id = save_backtest_analysis(scored, sim_date)
                    break
                except Exception as e:
                    if attempt < MAX_RETRIES - 1:
                        time.sleep(2)
...
def print_summary():
    ...
    # Accuracy — VIABLE predictions that actually went up
    cur.execute("""
        SELECT
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Tickers to backtest (default: all)"
    )
    parser.add_argument(
        "--days",
        type=int,
