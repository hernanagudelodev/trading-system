...
Dependencies:
    ...
    OBLIGATORIAS (el run explota si faltan — a propósito):
        ...
        ACCOUNT_NLV             capital base (fallback; el real sale del broker)
        LIVE_TRADING_ENABLED    'true' habilita live (capa 1 del interruptor).
                                Cualquier otro valor / ausente -> solo paper.
        EXECUTOR_ENV            'sandbox' | 'production' (obligatoria si live opera)

    def: un solo proceso corre AMBOS libros. Ya NO existe TRADING_MODE — que live
    opere lo decide el interruptor (LIVE_TRADING_ENABLED + kill-flag en DB).
"""
...
def run_claude_analysis(market_ctx, scanner_report):
    ...
    # def · decision 1: el LLM NO ve la cartera. No se le pasan posiciones abiertas
    # ni cierres. Los gates de concentracion/riesgo son DETERMINISTAS y viven en
    # cada executor, DESPUES de que el LLM decide — el LLM no los anticipa ni los
    # razona. Los cierres los maneja el monitor por reglas, no el LLM.
...
def sanear_recomendaciones(new_trades):
    """
    Prepara las recomendaciones del LLM para ejecutar. UNA vez, resultado usado por
    los DOS libros. Saca completitud y arregla la expiracion (el LLM inventa
    sabados). NO aplica gates de cartera: eso vive en cada executor.

    Devuelve (intents, descartes): OpenIntent listos + motivos de descarte.
    """
    from executor import OpenIntent
    import option_selector

    intents, descartes = [], []
    for rec in (new_trades or []):
        ticker      = (rec.get("ticker") or "").upper()
        strike_low  = rec.get("strike_low")
        strike_high = rec.get("strike_high")
        debit       = rec.get("debit")
        rationale   = rec.get("rationale", "")

        if not all([ticker, strike_low is not None, strike_high is not None,
                    debit is not None]):
            descartes.append(f"{ticker or '???'}: recomendacion incompleta, se omite")
            continue

        real_exp = option_selector.get_real_expiration(ticker)
        if real_exp is None:
            descartes.append(f"{ticker}: sin expiracion real en la cadena, se omite")
            continue
        expiration = real_exp.isoformat()
        if rec.get("expiration") and rec["expiration"] != expiration:
            print(f"  [exp-fix] {ticker}: LLM dijo {rec['expiration']}, "
                  f"se usa la real {expiration}")

        intents.append(OpenIntent(
            ticker=ticker, strike_low=float(strike_low),
            strike_high=float(strike_high), expiration=expiration,
            debit=float(debit), rationale=rationale, context_json=None,
        ))
    return intents, descartes


def ejecutar_aperturas(intents):
    """
    Ejecuta aperturas en los DOS libros. SOLO abre — el P&L, stops y cierres son
    del MONITOR (proceso aparte). Orden: live -> run_sync -> paper (decision 7).

    Una decision del LLM (los intents), dos ejecuciones. Que paper abra y live no
    (o al reves) solo puede venir de un gate DETERMINISTA de cartera o del
    interruptor de live, nunca de que el LLM penso distinto (fix del bug de 9am).
    """
    from executor import LiveExecutor, PaperExecutor

    live, paper = LiveExecutor(), PaperExecutor()
    res = {"live": [], "paper": [], "live_abiertas": 0, "paper_abiertas": 0}

    if not intents:
        print("  [aperturas] no hay intents que ejecutar.")
        return res
    ...
    if res["live_abiertas"] > 0:
        ...
        print(f"\n  [aperturas] SYNC — live no abrio nada, sin sync.")
    ...
    print(f"\n  [aperturas] resumen · live: {res['live_abiertas']}/{len(intents)} "
          f"· paper: {res['paper_abiertas']}/{len(intents)}")
    return res
...
def send_run_summary(market_ctx, analysis, results, run_time):
    ...
    # def: un proceso, dos libros. Tag fijo; el detalle de que abrio cada libro
    # va en el cuerpo del summary (results['opened'] paper, results['live_opened']).
    tag     = "🔴 LIVE+📄 PAPER"
    verdict = market_ctx.get("verdict", "N/A") if market_ctx else "N/A"
...
def _save_log_to_db(market_ctx, analysis, results, run_time, slot="unknown"):
    ...
    try:
        ...
        """, (
            ...
            full_log,
            int(run_time),
            "def",   # def: un proceso corre ambos libros, no hay modo unico
        ))
...
def market_status():
    """
    Estado de la sesion de NYSE segun Tastytrade — la misma fuente que ejecuta
    las ordenes. Devuelve el string crudo del broker ('Open' | 'Closed' |
    'Pre-market' | 'Extended') o None si no se pudo consultar.

    Primitiva unica de la consulta al endpoint de sesiones: is_market_open() y el
    snapshot del monitor derivan de esta, para no pegarle dos veces al broker por
    lo mismo.
    """
    async def _fetch():
        ...
        return sessions[0].status if sessions else None

    try:
        status = asyncio.run(_fetch())
    except Exception as e:
        print(f"  [mercado] no se pudo consultar Tastytrade ({e})")
        return None
    if status is None:
        print("  [mercado] sin estado de Tastytrade")
        return None
    return str(status)
...
def is_market_open():
    """
    True SOLO si NYSE esta en sesion regular ('Open'). Reemplaza a is_market_day
    (que solo miraba weekday y no sabia de feriados: el 01-sep, Labor Day, el
    auto_run corrio y trato de abrir con precios del viernes).

    FAIL-CLOSED: si market_status() no confirma 'Open' (red, sesion, API o None),
    devuelve False y el run se salta. El daño a evitar es operar sobre datos stale
    de un mercado cerrado; ante la duda, no operar. Es lo OPUESTO a _ya_corrio_hoy,
    que fail-opens a proposito.
    ...
    is_open = (status == "Open")
    print(f"  [mercado] NYSE segun Tastytrade: {status} — {'corre' if is_open else 'NO corre'}")
    return is_open
...
def main():
    ...
    # def: un solo proceso corre los DOS libros. No hay TRADING_MODE. El tag es
    # fijo; que live opere o no lo decide el interruptor dentro del LiveExecutor.
    RUN_TAG = "🔴 LIVE+📄 PAPER"
    ...
    try:
        ...
        # Step 5 — Claude analysis (el LLM NO recibe cartera: decision 1)
        print("\n  [5/6] Calling Anthropic API...")
        analysis = run_claude_analysis(market_ctx, scanner_report)
        ...
        # Step 6 — Sanear recomendaciones (una vez) y ejecutar aperturas en ambos
        # libros. Los CIERRES sugeridos por el LLM se notifican pero NO se ejecutan
        # (decision del proyecto: el LLM no cierra; ver cierres abajo).
        print("\n  [6/6] Sanear + ejecutar aperturas...")
        new_trades = (analysis or {}).get("new_trades", [])
        intents, descartes = sanear_recomendaciones(new_trades)
        for d in descartes:
            print(f"  [sanear] {d}")
        apert = ejecutar_aperturas(intents)

        # Estructura de results compatible con send_run_summary / save_log.
        # 'opened' = aperturas de PAPER (el libro que siempre corre); los cierres
        # los maneja el monitor, aca no se cierra nada.
        results = {
            "opened":  [t for t, ok in apert["paper"] if ok],
            "closed":  [],
            "errors":  [],
            "live_opened":  [t for t, ok in apert["live"] if ok],
            "descartes": descartes,
        }
