...
def ensure_tables():
    ...
    # ALTER de positions (libro LIVE): tolerante a que la tabla no exista todavia.
    # En v2 el flujo de paper no crea `positions`; sin el guard, este ALTER caeria
    # y frenaria la creacion de paper_positions. Se aplica cuando exista positions.
    cur.execute("""
        DO $$
        BEGIN
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS trade_context (
            ...
            -- El FK a positions (libro LIVE) se omite en v2 hasta que exista esa
            -- tabla; position_id queda como columna suelta y el flujo live la usa.
            position_id         INTEGER,
...
def cmd_paper_buy(ticker, strike_low, strike_high, expiration_str, debit,
                  ...
                  price_at_open=0.0, strategy=None, selection_id=None):
    """
    Registra un spread en paper y guarda el contexto del trade.

    strategy: la estructura elegida por la Capa de Seleccion (bidireccional). Si
        viene, MANDA. Si es None, se deriva del signo del debito (SOLO alcista,
        compatibilidad con el CLI viejo) -> nunca produce una bajista por accidente.

    El SIGNO del debito determina debito/credito (correcto para las 4 spreads); el
    NOMBRE y el LADO (put/call) salen de la ESTRUCTURA, no del signo. La asignacion
    long/short de las patas depende de la geometria: alcista -> long=strike bajo;
    bajista -> long=strike alto (el monitor pricea con el signo segun cual es long).
    Longs de 1 pata (Long Call/Put) NO pasan por aca todavia.
    """
    ensure_tables()
    ...
    # Estructura: la explicita si viene; si no, la alcista por signo (compat CLI).
    if strategy is None:
        strategy = "Bull Put Spread" if debit < 0 else "Bull Call Spread"

    is_put  = strategy in ("Bull Put Spread", "Bear Put Spread")    # familia put
    is_bull = strategy in ("Bull Call Spread", "Bull Put Spread")   # geometria alcista

    # Debito/credito por SIGNO (correcto para las 4 spreads).
    if debit < 0:
        net_credit = abs(debit)
        total_cost = round(-net_credit * 100, 2)
        max_profit = round(net_credit * 100, 2)
        max_loss   = round((spread_width - net_credit) * 100, 2)
        econ = f"Credito: ${net_credit:.2f}. Max profit: ${max_profit:.2f}. Max loss: -${max_loss:.2f}."
    else:
        total_cost = round(debit * 100, 2)
        max_profit = round((spread_width - debit) * 100, 2)
        max_loss   = round(debit * 100, 2)
        econ = f"Debit: ${debit:.2f}. Cost: ${total_cost:.2f}. Max profit: ${max_profit:.2f}."

    auto_notes = f"Paper trade — {strategy} ${strike_low}/${strike_high} exp {expiration}. {econ}"
    if notes:
        auto_notes += f" | {notes}"

    # Simbolos OCC por ESTRUCTURA: lado put/call por familia; long/short por geometria.
    option_type = "put" if is_put else "call"
    long_strike, short_strike = (strike_low, strike_high) if is_bull else (strike_high, strike_low)
    sym_long  = build_occ_symbol(ticker, expiration, option_type, long_strike)
    sym_short = build_occ_symbol(ticker, expiration, option_type, short_strike)
    ...
    """, (
        ...
        auto_notes,
        sym_long, sym_short, option_type, selection_id,
    ))
    pos_id = cur.fetchone()[0]
    conn.commit()
    cur.close()
    ...
    print(f"     {ticker} {strategy} ${strike_low}/${strike_high}")
